      ...
      <div className="max-w-6xl mx-auto space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  size="icon"
                  className={`h-10 w-10 shrink-0 ${showFilters ? 'bg-primary/10 text-primary' : 'bg-muted/50'}`}
            ...
            {showFilters && (
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 p-4 bg-muted/30 rounded-lg animate-in fade-in slide-in-from-top-2">
                <div className="space-y-1.5">
                  <label className="text-[10px] uppercase font-bold text-muted-foreground ml-1">Filter by Form</label>
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] uppercase font-bold text-muted-foreground ml-1">Filter by Status</label>
                ...
                <div className="flex items-end">
                  <Button variant="ghost" size="sm" className="w-full text-xs gap-1.5 text-muted-foreground" onClick={clearFilters}>
                    <X className="h-3 w-3" />
                    Clear all filters
                  </Button>
                </div>
        ...
        {isAllResponsesLoading && (
          <div className="text-center py-12">
            ...
            <div className="text-muted-foreground animate-pulse font-medium">Synchronizing responses...</div>
          </div>
        )}
        ...
        {filteredGroups.length > 0 ? (
          <div className="space-y-6">
            {filteredGroups.map((group) => (
              <Card key={group.formId} className="border-none shadow-md overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-500">
                <CardHeader className="bg-muted/30 pb-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                      <CardTitle className="text-lg">{group.formTitle || 'Untitled Form'}</CardTitle>
                    ...
                    <table className="w-full text-left text-sm">
                      <thead className="bg-muted/20 text-muted-foreground uppercase text-[10px] font-bold tracking-widest">
                        <tr>
                          ...
                          <th className="px-6 py-3 min-w-[200px]">Answer Summary</th>
                            ...
                            <td className="px-6 py-4 align-top">
                              <div className="space-y-1.5">
                                {Object.entries(response.answers)
                                  .slice(0, expandedAnswers[response.id] ? undefined : 2)
                                  .map(([k, v]) => (
                                ...
                                {Object.keys(response.answers).length > 2 && (
                                  ...
                                  >
                                    ...
                                    <ChevronDown className={`h-3 w-3 transition-transform ${expandedAnswers[response.id] ? 'rotate-180' : ''}`} />
                                  </button>
                                )}
                              </div>
                            </td>
                            <td className="px-6 py-4 align-top">
                              <Badge variant={response.status === 'Completed' ? 'outline' : 'secondary'} className={`text-[9px] font-bold px-1.5 py-0 ${response.status === 'Completed' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-orange-50 text-orange-700 border-orange-200'
                                }`}>
                                {response.status}
                              </Badge>
                            </td>
          ...
          !isAllResponsesLoading && (
            <div className="text-center py-24 border-2 border-dashed rounded-2xl bg-muted/5">
              ...
              <h3 className="text-xl font-bold">No matches found for your filters</h3>
